...
def sanitized_bet(row: dict[str, str], index: int, simulated: bool) -> dict[str, Any]:
    ...
    record = {
        ...
        "marketSettlementStatus": row.get("market_settlement_status") or ("unresolved" if status in {"open", "pending_order", "unfilled", "partial_order"} else status),
        ...
        "marketResult": row.get("market_result") or row.get("expiration_value") or ("open" if status in {"open", "pending_order", "unfilled", "partial_order"} else ""),
